...
A signal exposes its readouts, and the reward readout carries \(w_r\) and \(b\) directly. No probe, no fitting.
...
Projecting an activation onto that direction is a dot product, \(w_r^{\top} h + b\). Hand it any hidden state and you get the reward that state would produce on its own. Every observational tool in the library is built from exactly this operation, applied to different activations: to each layer's residual stream (the [reward lens](../instruments/lens-crystallization.md)), to each component's output ([attribution](../instruments/attribution.md)), to each dictionary feature ([feature-reward alignment](../instruments/feature-alignment.md)), to an extracted concept direction ([concept dose-response](../instruments/concept-dose-response.md)).

![A residual-stream state projected onto the reward direction, with a level set of constant reward.](../assets/figures/reward-projection-light.svg#only-light){ .rl-fig .rl-fig--hero }
![A residual-stream state projected onto the reward direction, with a level set of constant reward.](../assets/figures/reward-projection-dark.svg#only-dark){ .rl-fig .rl-fig--hero }
