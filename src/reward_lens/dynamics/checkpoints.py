...
checkpoints carried deliberately through a second epoch) is a few-hundred-GPU-hour build that this
library does not perform. Its terms are recorded in `RM_PYTHIA_RELEASE_INTENT` and a chain built
elsewhere is read back through `from_hf_revisions` (DESIGN 4.5, 4.4 M9); no entry point here stands
in for the training loop. The CPU-provable vehicle is `synthetic_planted_sequence`, a handful of tiny
...
class CheckpointSequence:
    ...
    def verify_fingerprints(self) -> ChainVerification:
        ...
        The third argument is the adapter id the loader recorded, read back off the signal's lineage,
        not a value derived here. That matters for whether this check can fail at all. It used to pass
        the adapter object's class name, which is the constant ``"GraderAdapter"`` on every path,
        into a fingerprint that hashed the base model directory: swap the adapter under a checkpoint
        and both sides of the comparison moved together, so the deep check was structurally incapable
        of catching the one thing it exists to catch. Recomputing from the recorded id, against a
        fingerprint that now hashes the adapter tensors, makes it capable (BLK-002, BLK-034).
        """
        ...
        for cp in self._checkpoints:
            ...
            lineage = getattr(meta, "lineage", None) or {}
            adapter_id = str(lineage.get("adapter_id", "") or "")
            ...
            recomputed = compute_fingerprint(model, tokenizer, adapter_id)
