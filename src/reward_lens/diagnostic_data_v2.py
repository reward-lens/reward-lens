"""
Diagnostic Pair Set v2 — populated, ≥30 pairs/dimension × 12 dimensions.

The v1 :mod:`diagnostic_data` shipped 2-4 hand-written pairs per dimension.
That was deliberately tiny (it ships in the package wheel) but it cannot
support population-scale faithfulness or hacking-at-scale; aggregate stats
on n=4 are dominated by sampling noise.

This v2 set is generated by applying templated mutations to a hand-written
seed of ≥5 pairs per dimension. Mutations are dimension-specific and
preserve the "preferred and dispreferred differ only on one axis" property:
...
The seed pairs are human-written. The mutations are deterministic textual
transforms — they preserve the labelled differential while extending the
sample size enough that bootstrap CIs are interpretable.

The labelled "preferred" is the label intended by the dataset designer;
this set is *not* a ground-truth oracle for any specific model. A model's
disagreement with the label is a result, not a bug.
"""
...
def _generate(seeds: list[tuple[str, str, str, str]], dimension: str,
              n_target: int) -> list[PreferencePair]:
    ...
    for prompt, pref, dispref, desc in seeds:
        out.append(PreferencePair(
            prompt=prompt, preferred=pref, dispreferred=dispref,
            dimension=dimension, description=desc,
        ))
...
def get_pairs_v2(dimensions: Optional[list[str]] = None,
                 n_per_dim: int = _PAIRS_PER_DIM) -> list[PreferencePair]:
