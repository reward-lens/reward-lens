...
class DivergenceAwarePatching(ActivationPatcher):
    ...
    def patch_with_divergence_check(
        ...
        for i, (name, ctype, layer_idx) in enumerate(
            zip(
            ...
            # Get the source activation being patched in. The divergence is
            # measured on that activation, so only the source cache is needed.
            if mode == "noising":
