...
class ConflictReport:
    ...
    relationship_matrix: (
        np.ndarray
    )  # n_terms × n_terms, encoded as 0=aligned, 1=orthogonal, 2=conflict
    ...
    def print_summary(self) -> None:
        ...
        risk_level = (
            "🔴 HIGH"
            if self.monitorability_risk > 0.6
            else ("🟡 MODERATE" if self.monitorability_risk > 0.3 else "🟢 LOW")
        )
        ...
        for analysis in self.pairwise_analysis:
            icon = (
                "🔴"
                if analysis.relationship == RelationshipType.IN_CONFLICT
                else ("⚪" if analysis.relationship == RelationshipType.ORTHOGONAL else "🟢")
            )
    ...
    def plot(
        ...
        ax2.imshow(self.relationship_matrix, cmap=cmap, vmin=0, vmax=2)
...
class RewardConflictAnalyzer:
    ...
    def analyze_conflicts(
        ...
        if n_pairs > 0:
            ...
            max_conflict = max((pa.conflict_severity for pa in pairwise_analysis), default=0)
    ...
    def analyze_multi_objective_model(
        ...
        try:
            ...
            if reward_weight.dim() > 1:
                ...
                terms = {
                    name: reward_weight[i] for i, name in enumerate(objective_names[:n_objectives])
                }
            else:
    ...
    def _pair_recommendations(
        ...
        if relationship == RelationshipType.IN_CONFLICT:
            ...
            recs.append(
                f"Terms '{term1}' and '{term2}' are orthogonal. Safe to optimize independently."
            )
        else:  # ALIGNED
            if cos_sim > 0.8:
                recs.append(
