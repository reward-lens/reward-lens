...
class ConceptAlignmentReport:
    ...
    def plot(
        ...
        colors = ['#2196F3' if a > 0.3 else '#F44336' if a < -0.3 else '#9E9E9E'
                  for a in alignments]
        ...
        scatter = ax2.scatter(separabilities, risks, c=alignments, cmap='RdYlBu',
                              vmin=-1, vmax=1, s=100, alpha=0.7)
        
        for i, name in enumerate(names):
            ax2.annotate(name, (separabilities[i], risks[i]),
...
class ConceptExtractor:
    ...
    def _generate_recommendations(
        ...
        if "agreement" in reward_aligned or "sycophancy" in [c.name.lower() for c in concepts if c.is_reward_aligned]:
            recs.append(
        ...
        if "verbosity" in reward_aligned:
            recs.append(
                "Verbosity aligns with reward. Model may pad responses unnecessarily."
            )
