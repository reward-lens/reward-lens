...
from reward_lens.runtime.hooks import (
    CaptureMount,
    LeafCutMount,
...
class HFRuntime:
    ...
    def forward_with_capture(
        ...
        Position resolution (BLK-005). ``spec.position`` is resolved per row through
        ``PositionSpec.resolve``, by ``hooks.resolve_capture_positions``, into token indices in the
        batch's padded coordinates. When every row resolves to exactly one index and
        ``spec.full_sequence`` is not set, the mount gathers that index and stores ``(B, d)`` per
        site; that covers ``final``, a scalar ``explicit`` and a resolved ``judgment``, and it is
        the memory-light path the battery uses everywhere. When any row resolves to more than one,
        the whole sequence is stored, because ``CaptureMount``'s gather is one index per row and a
        ragged ``(B, K)`` is not a tensor. **Either way the resolved indices go onto
        ``Capture.positions``**, which is what that field promises and what used to come back
        empty for every kind but ``final``.

        Captures and interventions share this mounting path, so a captured Observable composes with
        a mounted intervention unchanged.
        """
        import torch
        ...
        mask = batch.attention_mask.to(self.device)
        final_pos = self._final_positions(mask)
        positions_list = resolve_capture_positions(batch, spec.position, final_positions=final_pos)
        one_per_row = all(len(p) == 1 for p in positions_list)
        gather = one_per_row and not spec.full_sequence
        positions = (
            torch.tensor([p[0] for p in positions_list], dtype=torch.long, device=self.device)
            if gather
            else None
        )
        mount = CaptureMount(
