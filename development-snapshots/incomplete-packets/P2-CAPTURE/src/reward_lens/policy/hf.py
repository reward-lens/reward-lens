...
class HFPolicyRuntime:
    ...
    def forward_with_capture(
        ...
        on this side too. Position resolution follows the grader path exactly, through the same
        `hooks.resolve_capture_positions`: `spec.position` is resolved per row through
        `PositionSpec.resolve`, one index per row is gathered and stored ``(B, d)``, more than one
        on any row keeps the whole sequence because the mount's gather is one index per row, and
        the resolved indices are recorded on `Capture.positions` either way.

        The two runtimes were structurally identical at the defect BLK-005 names and they share
        one resolver at the repair, because the alternative is the same coordinate meaning two
        things depending on which side of the run reads it.
        """
        import torch
