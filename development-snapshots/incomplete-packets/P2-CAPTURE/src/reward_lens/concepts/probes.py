...
def capture_probe_inputs(
    ...
    *,
    ...
    ``position`` is a ``PositionSpec`` and defaults to the final token. Before BLK-005 this module
    contained no occurrence of ``PositionSpec`` at all, so a probe fitted here could only ever read
    the last token of the full sequence, which is a completion position. A probe meant to predict
    something about a rollout that has not happened yet has to read before it, and both sides of a
    pair are read at the same coordinate so the per-pair differential stays a differential.
    """
    ...
    caps_chosen = _capture_matrix(signal, chosen_items, sites, position) if chosen_items else {}
    caps_rejected = (
        _capture_matrix(signal, rejected_items, sites, position) if rejected_items else {}
    )
...
def _capture_matrix(
    ...
    """Activations at ``sites`` for ``items`` as numpy fp32, at ``position`` (torch-gated).

    ``position`` is a ``PositionSpec``; ``None`` means the final token, which is what this took
    unconditionally before BLK-005 and what every probe that reads a completed response wants. A
    probe that has to read before generation needs a different coordinate, and this is where it
    enters the probe path.
    """
    from reward_lens.measure.battery._common import capture_sites

    tensors = capture_sites(signal, items, sites, position=position, dtype="float32")
