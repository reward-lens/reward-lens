...
def capture_sites(
    ...
    *,
    ...
    With ``full_sequence=False`` (the default) each tensor is the activation at the resolved
    position, ``(B, d)`` (or ``(B, d_head)`` for a head site); with ``full_sequence=True`` it is
    the whole sequence ``(B, T, d)``, which the patching mechanics need for a source activation.
    fp32 by default.

    ``position`` defaults to ``PositionSpec("final")``, which is what every existing caller wants
    and what this function used to hard-code with no way past it. That hard-coding is BLK-005: the
    design reads at a coordinate inside the prompt, and with no parameter here the public path
    could not express it, so link 2 would have measured a text-description task at the last
    completion token rather than a pre-generation propensity. A kind that resolves to more than
    one index per row returns the whole sequence, because ``CaptureMount`` gathers one index per
    row; ``signal.capture`` returns the resolved indices on ``Capture.positions`` and a caller
    that needs them should read the ``Capture`` rather than this convenience wrapper.
    """
    spec = CaptureSpec(
        sites=sites,
        position=position if position is not None else PositionSpec("final"),
