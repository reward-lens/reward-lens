...
def test_edited_scores_chunking_is_exact_on_a_40_row_view(signal):
    """A 40-row view scored in chunks reproduces the per-chunk scores bit for bit.

    ``run_edited_scores`` chunks its forwards at ``batch_size`` so a population-scale view is
    never collated into one batch. The chunk mechanics must add nothing: scoring the view at
    ``batch_size=16`` is bit-identical to scoring each 16-row slice on its own and
    concatenating, and the default batch size takes the same path. Against a single 40-row
    forward the match is to fp32 ULP rather than to the bit, because CPU BLAS may block a
    matmul differently for a different batch dimension; that is a property of the forward
    (``HFRuntime.final_head_inputs`` shares it), so it is pinned at tolerance here, not hidden.
    """
    d_model = signal.readout("reward").vector.shape[0]
    rng = np.random.default_rng(7)
    direction = rng.standard_normal(d_model).astype(np.float32)
    compiled = EditIntervention(direction=direction, strength=1.0).compile(signal)
    view = [(f"Question {i}?", f"Answer number {i} follows here.") for i in range(40)]

    chunked = run_edited_scores(signal, compiled, view, batch_size=16)
    default = run_edited_scores(signal, compiled, view)
    per_chunk = np.concatenate(
        [run_edited_scores(signal, compiled, view[a : a + 16], batch_size=None) for a in (0, 16, 32)]
    )
    whole = run_edited_scores(signal, compiled, view, batch_size=None)

    assert chunked.shape == whole.shape == (40,)
    assert np.array_equal(chunked, per_chunk), "chunking changed the per-chunk scores"
    assert np.array_equal(chunked, default), "the default batch size took a different path"
    assert np.allclose(chunked, whole, rtol=0, atol=1e-6), (
        "chunked scores drifted from the single-batch forward beyond ULP noise"
    )


def test_edited_scores_recover_from_oom_by_halving(signal, monkeypatch):
    """A chunk that fails with a CUDA-style OOM is halved and retried, preserving order.

    CPU cannot raise a real CUDA OOM, so the runtime's forward is wrapped to refuse batches
    larger than 8 rows with the message the halving logic keys on. Halving 20 rows bottoms out
    at the same 5-row slices an explicit ``batch_size=5`` run forwards, so the recovered
    scores must equal that run bit for bit.
    """
    w_r = signal.readout("reward").vector.detach().cpu().numpy()
    compiled = EditIntervention(direction=w_r, strength=1.0).compile(signal)
    view = [(f"Question {i}?", f"Answer number {i} follows here.") for i in range(20)]
    want = run_edited_scores(signal, compiled, view, batch_size=5)

    real_forward = signal.runtime.forward

    def refusing_forward(batch):
        if batch.input_ids.shape[0] > 8:
            raise RuntimeError("CUDA out of memory (simulated)")
        return real_forward(batch)

    monkeypatch.setattr(signal.runtime, "forward", refusing_forward)
    got = run_edited_scores(signal, compiled, view, batch_size=20)
    assert np.array_equal(want, got), "halve-and-retry changed or reordered the scores"
