...
def _pooled_head_inputs(runtime: Any, sub: list[Any]) -> "torch.Tensor":
    """Final-token head inputs for one chunk of tokenized items, halving on CUDA OOM.

    Mirrors ``HFRuntime._final_head_inputs_chunk`` (the v1 halve-and-retry mechanic): a chunk
    that does not fit is split in two and each half retried, recursing down to single items
    before giving up. Any hooks the caller installed stay in place across the retries, so a
    halved forward is still the edited forward. Rows come back one per item in input order.
    """
    import torch

    from reward_lens.runtime.hf import _is_oom

    try:
        raw = runtime.forward(runtime.collate(sub))
        ...
        mid = len(sub) // 2
        first = _pooled_head_inputs(runtime, list(sub)[:mid])
        second = _pooled_head_inputs(runtime, list(sub)[mid:])
    ...
    return head_input[idx, final_pos]
...
def run_edited_scores(
    ...
    readout vector, not an activation, so this runner forwards the view (installing any activation
    ...
    ``batch_size`` chunks the forwards so a population-scale view is never collated into one
    batch (``None`` forwards the whole view at once); a chunk that hits CUDA OOM is halved and
    retried, the same recovery ``HFRuntime.final_head_inputs`` uses. The chunk mechanics are
    exact: rows are concatenated in input order and projected once, so scoring a view in
    chunks is bit-identical to scoring each chunk on its own. Against a single whole-view
    forward the scores agree to fp32 ULP, not to the bit, because BLAS kernels may block a
    matmul differently for a different batch dimension; that caveat is the forward's, shared
    with ``HFRuntime.final_head_inputs``, not the chunking's.
    """
    import torch
    ...
    try:
        ...
        while start < len(tokenized):
            pooled_parts.append(_pooled_head_inputs(runtime, tokenized[start : start + chunk]))
            start += chunk
